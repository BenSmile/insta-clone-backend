...
const createPost2 = async (data, user) => {
    const foundUser = await getUserByEmail(user);
    console.log("username -", foundUser.username);
    const params = Post.put({
        id: uuidv4(),
...
const getOneLike = async (post, username) => {
    const params = {
        TableName: 'like-table',
        FilterExpression: "#post = :post and #username = :username",
...
const likePost = async (post, user) => {
    ...
    let message = '';
    let response = {};
    const userFound = await getUserByEmail(user);
    const isLiked = await doILikeThePost(post, user);
    if (isLiked) {
        const findLikePost = await getOneLike(post, user);
        console.log('findLikePost-> ', findLikePost[0].id);
        const params = Like.delete({
            id: findLikePost[0].id,
            type: "LIKE",
        });
        await docClient.delete(params).promise();
        like = -1;
        message = `${userFound.username} unliked a post`;
    } else {
...
const doILikeThePost = async (post, user) => {
    const likes = await getOneLike(post, user);
